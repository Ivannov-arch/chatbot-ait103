...
import { createKnowledgeItem, updateKnowledgeItem, deleteKnowlegeItem } from "@/services/knowledgeService";
...
function CategoryBadge({ module }: { module: string }) {
  ...
  return (
    <span
      className={`inline-block px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${cfg.bg} ${cfg.text}`}
    >
      {cfg.label}
    </span>
  );
...
const inputCls =
  "w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all";
...
export default function KnowledgeBaseCMS() {
  ...
  const renderFormFields = () => (
    <>
      ...
      <Field label="Keywords (comma-separated)" hint="Separated by commas — used by the keyword-matching algorithm.">
        <input
          type="text"
          value={formKeywords}
  ...
  return (
    <div className="space-y-6 max-w-6xl relative">
      ...
      {toast && (
        <div
          className={`fixed top-5 right-5 z-[99999] flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl text-white text-sm font-semibold animate-in slide-in-from-right-4 transition-all ${
      ...
      {/* ── Control Bar ── */}
      <div className="bg-white px-5 py-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center gap-4">
        {/* Search */}
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 border border-slate-200 rounded-full text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
          />
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <select
            ...
            className="flex-1 md:w-48 px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
          >
          ...
          <button
            ...
            className="flex-shrink-0 flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-full shadow-sm hover:shadow-md transition-all cursor-pointer"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-20 text-center">
            ...
            <p className="font-bold text-slate-700 text-sm">No Q&amp;A items found</p>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-500 text-[10px] font-bold uppercase tracking-wider border-b border-slate-200">
                  ...
                  <th className="py-3 px-4 w-72">Question</th>
                  <th className="py-3 px-4">Answer</th>
                  <th className="py-3 px-4 w-52">Keywords</th>
                ...
                {filteredItems.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Question */}
                      <td className="py-3 px-4 font-semibold text-slate-800 text-xs max-w-[260px] break-words leading-snug">
                        {item.question}
                      </td>

                      {/* Answer (truncated) */}
                      <td className="py-3 px-4 text-xs text-slate-500 max-w-[300px] leading-relaxed">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {item.keywords && item.keywords.length > 0 ? (
                            item.keywords.map((kw, i) => (
                              <span
                                key={i}
                                className="px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-600 border border-slate-200 font-mono text-[9px] font-medium"
                              >
                                {kw}
                              </span>
                            ))
                      ...
                      <td className="py-3 px-4 text-right space-x-1">
                        <button
                          ...
                          title="Edit item"
                          className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-indigo-500 hover:bg-indigo-600 hover:text-white transition-all cursor-pointer"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button
                          ...
                          title="Delete item"
                          className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-500 hover:bg-red-600 hover:text-white transition-all cursor-pointer"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="bg-slate-900 text-white px-6 py-4 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm">Add New Q&amp;A Item</h3>
              </div>
              <button
            ...
            <form onSubmit={handleAdd} className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl shadow transition-all cursor-pointer"
                >
                  Insert Q&amp;A
                </button>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="bg-slate-900 text-white px-6 py-4 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm">Edit Q&amp;A Item</h3>
              </div>
              <button
            ...
            <form onSubmit={handleEdit} className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl shadow transition-all cursor-pointer"
                >
                  Save Changes
                </button>
              </div>
      ...
      {isDeleteOpen && currentItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[99999]">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-red-700 text-white px-6 py-4 flex items-center gap-2">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl text-xs space-y-1">
                <p className="font-bold text-slate-800">{currentItem.question}</p>
                <p className="text-slate-500 leading-relaxed line-clamp-3">{currentItem.answer}</p>
              </div>
              <div className="flex justify-end gap-3 pt-1">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-xl shadow transition-all cursor-pointer"
                >
                  Delete Permanently
                </button>
              </div>
