...
# Jalankan: python scripts/text_to_json.py campus_life database/seeds/campus_life_raw.txt
#
...
def parse_qa_file(module: str, txt_path: str):
    ...
    for block in blocks:
        entry = {"module": module, "question": "", "answer": {}}
        for line in block.splitlines():
            if line.startswith("Q:"):
                entry["question"] = line[2:].strip()
            elif line.startswith("A:"):
                entry["answer"]["text"] = line[2:].strip()
            elif line.startswith("K:"):
                entry["answer"]["keywords"] = [k.strip() for k in line[2:].split(",") if k.strip()]
            
        if entry["question"] and entry["answer"]["text"]:
            items.append(entry)
        else:
            skipped += 1
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("Example: python scripts/text_to_json.py campus_life database/seeds/campus_life_raw.txt")
        sys.exit(1)
